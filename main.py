...
from battleship.Rules import Rules
#В Rules можно настраивать параметры игры
...
class Game(object):
    ...
    ships_rules = Rules.ships_rules
    field_size = Rules.field_size
    ...
    def get_input(self, input_type):
        ...
        if input_type == "ship_setup":
            ...
            if x not in Game.letters or not y.isdigit() or int(y) not in range(1, Game.field_size + 1) or \
                ...
                self.message.append('Данные передаются на первую позицию коробля')
                self.message.append('в формате (строка)(столбец)(H или V)')
                self.message.append('Где H - ориентация по горизонтали,')
                self.message.append('V - ориентация по вертикали')
                self.message.append('Запрос всегда содержит длинну коробля')
                self.message.append('Например, если запрос корабль 2: 12H')
                self.message.append('Соотвественно корабль будет расположен')
                self.message.append('По горизонтали в координатах 12 - 13')
                return 0, 0, 0
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            Username = input()
            if len(Username) > 0:
                print("Начинаем игру БОТа против",Username,"!")
                break
            else:
                print("Имя должно содержать хотя бы один символ, попробуйте еще раз!")
        except:
            print("Неизвестная ошибка, попробуйте еще раз!")

    while True:
        print("Расставить корабли автоматически: ДА либо YES, или вручную: НЕТ либо NO? ")
        try:
            auto_ship_str = str(input()).upper()
        except:
            auto_ship_str = None

        if auto_ship_str == 'ДА' or auto_ship_str == 'YES':
            str_auto_ship = True
            break
        elif auto_ship_str == 'НЕТ' or auto_ship_str == 'NO':
            str_auto_ship = False
            break
        else:
            print("Ответ не понятен, попробуйте еще раз!")

    players = []
    players.append(Player(name=Username, is_ai=False, auto_ship=str_auto_ship, skill=1))
    players.append(Player(name='БОТ', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        try:
            ...
            if game.status == 'in game':
                ...
                game.current_player.message.append('{}! Ждём приказа: '.format(game.current_player.name))
                game.draw()
            ...
            print('Неизвестная ошибка, игра завершена!')
            break
